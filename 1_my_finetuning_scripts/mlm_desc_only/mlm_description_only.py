...
LOGFILE_PATH = f'logfiles/mlm_{PRETRAINED_MODEL}_descriptions_only_{LANGUAGE}_test.log'
STDOUT_PATH = f'logfiles/mlm_{PRETRAINED_MODEL}_descriptions_only_{LANGUAGE}_test.out'
CHECKPOINTING_PATH = f'/cluster/scratch/yakram/{LANGUAGE}_models/{PRETRAINED_MODEL}_mlm_description_correct'
BEST_CHECKPOINT_PATH = f'/cluster/scratch/yakram/{LANGUAGE}_models/{PRETRAINED_MODEL}_mlm_description_correct_best'
CHUNK_SIZE = 256
...
LOGGER.info(f'Loading models and the tokenizer for the pretrained model {PRETRAINED_MODEL}')
model = AutoModelForMaskedLM.from_pretrained(PRETRAINED_MODEL, cache_dir='/cluster/scratch/yakram/conda_env/')
tokenizer = AutoTokenizer.from_pretrained(PRETRAINED_MODEL, cache_dir='/cluster/scratch/yakram/conda_env/')
...
LOGGER.info(f'The first shapes of memconcept is {query_MemConcept_desc.shape}, skills is {query_SkillsHier_desc.shape}')
...
query_MemConcept_desc.dropna(inplace=True)
query_SkillsHier_desc.dropna(inplace=True)
LOGGER.info(f'The shapes of memconcept is {query_MemConcept_desc.shape}, skills is {query_SkillsHier_desc.shape}')
desc_data_all = pd.DataFrame(pd.concat([MemConcept_desc, SkillsHier_desc]))
LOGGER.info(f'Shape of data is {desc_data_all.shape}, total na are {desc_data_all.isnull().sum().sum()}')
# desc_data_all.dropna(inplace=True)
...
LOGGER.info(
    f'The total length of the input ids in tokenized dataset is {len(tokenized_datasets["train"]["input_ids"])}')
