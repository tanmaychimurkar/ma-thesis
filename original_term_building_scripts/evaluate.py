...
import os
# from evaluate_testerms_contextualized import get_fullterm
import numpy as np
import pandas as pd
...
def get_AP10(d, eval_dict, uri2preflabel, eval_level, eval_scheme, gradSel, lastIndex=10):
...
def evaluate(sample, eval_dict, uri2preflabel, query_results_dir, infiles, outdir, outfile, level, measure, evalscheme,
             gradient_selector, lastIndex):
    ...
    for file in infiles[0:]:
        ...
        with open(os.path.join(query_results_dir, file), 'rt', encoding='utf-8') as f:
            for line in f:
                d = json.loads(line)
                fullterm = get_fullterm(d)
                #		print(fullterm)
...
def main():
    for sample in ['random', 'challenge']:
        if sample == 'random':
            ...
            infiles_class = ['human_eval/random_sample_classes_evaluated_asg_eb_sc_mean.txt']
            infiles_uris = ['human_eval/random_sample_uris_evaluated_asg_eb_mean.txt']
            queryresultsdir = 'original_results/queryresults_random_sample'
            outdir = 'evaluation-random-sample'

            if not os.path.exists(outdir):
                os.makedirs(outdir)

            # for assessment of dropping context<h
            # queryresultsdir='queryresults_random_sample_woC'
            # infiles_uris = ['human_eval/uris_evaluated_w_woC_mean_2022-08-31.txt']
            # infiles_class =['human_eval/classes_evaluated_w_woC_mean_2022-09-01.txt']
            # outdir = 'evaluation-random-sample-2022-09-01-woC'

            # read in human annotations on class
            term_class_with_rater_mean_score = get_class_annotations(infiles_class)
            for k, v in term_class_with_rater_mean_score.items():
                print(repr(k), v)
            # read in human annotations on uri level
            term_uri_rater_mean_score = get_evaldict_terms(infiles_uris)

            # resultfiles from diff. queries with diff models.
            infiles = [f for f in os.listdir(queryresultsdir) if f.endswith('.bz2')]
            # infiles = [f for f in os.listdir(queryresultsdir) if f.endswith('.bz2')]
            print(infiles)
            # if not os.path.exists(outdir):
            #	os.makedirs(outdir)

            # get mAP,  with different configurations (class and uri-level, hard and soft eval, map@1, map@10, and with/withoug grad sel.)
            measure = 'mAP'
            levels = ['uri', 'class']
            evaldicts = [term_uri_rater_mean_score, term_class_with_rater_mean_score]
            eval_scheme = ['soft', 'hard']
            lastIndex = [1, 10, 10]
            gradSel = [False, True, False]
            for level, eval_dict in zip(levels, evaldicts):
                for evalscheme in eval_scheme:
                    for last_index, gradient_selector in zip(lastIndex, gradSel):
                        ...
                        outfile += '.jsonl'
                        print(outfile)
                        evaluate(sample, eval_dict, uri2pref, queryresultsdir, infiles, outdir, outfile, level, measure,
            ...
            show_terms(sample, term_uri_rater_mean_score, uri2pref, queryresultsdir, infiles, outdir, 'uri', measure,
                       'soft', True, 10)
            show_terms(sample, term_class_with_rater_mean_score, uri2pref, queryresultsdir, infiles, outdir, 'class',
                       measure, 'soft', True, 10)
            # #
            show_sims(sample, term_uri_rater_mean_score, uri2pref, queryresultsdir, infiles, outdir, 'sims_soft.tsv',
                      'uri', measure, 'soft',
                      True, 10)
            show_sims(sample, term_uri_rater_mean_score, uri2pref, queryresultsdir, infiles, outdir, 'sims_hard.tsv',
                      'uri', measure, 'hard',
                      True, 10)

        if sample == 'challenge':

            # old part: in case one needs to evaluate more terms because of new queries. check function, similar function for uris in code above.
            # t2c = {}
            # labels = get_classlabels('../skillontology/ESCO/classL4up_labels.jsonl')
            # indir = 'custom_sample_results'
            # infiles = [f for f in os.listdir(indir) if f.endswith('bz2')]
            # outdir = 'custom_eval'
            # outfile = 'custom_classes_toEval.tsv'
            # get_classes_to_eval(t2c, labels, indir, infiles, outdir, outfile)
            ...
            infiles = [
                'sbert_adaption/human_eval/challenge_sample_classes_evaluated_asg_eb_cs_mean.txt']
            term_class_with_rater_mean_score = get_class_annotations(infiles)

            # queryresultsdir = 'queryresults_random_sample_test'
            # outdir = 'evaluation-random-sample_test-2022-11-10'

            # get evaluations:
            queryresultsdir = 'queryresults_challenge_sample'
            # infiles = [f for f in os.listdir(queryresultsdir) if f.endswith('.bz2') and 'challenge' in f]
            infiles = [f for f in os.listdir(queryresultsdir) if f.endswith('.bz2')]
            print(infiles)
            outdir = 'evaluation-challenge-sample'
            if not os.path.exists(outdir):
                os.makedirs(outdir)
